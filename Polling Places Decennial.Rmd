...
cor(census_tracts_percentage$percent_black, census_tracts_percentage$nearest_polling_place)
cor(census_tracts_percentage$percent_black, census_tracts_percentage$five_nearest_polling_places)
...
```{r}
ggplot() + geom_histogram(aes(as.numeric(census_block_percentage$percent_black)))
```
...
```{r}
plot = ggplot() + geom_sf(data = census_block_percentage$geometry, colour = census_block_percentage$percent_black, size = 0.1)
plot + theme_minimal()
```
...
ggplot() + geom_sf(data = decile$geometry, aes(colour = decile$percent_black), size = 0.1) + scale_color_distiller(palette = "Spectral") + theme_minimal()
```
